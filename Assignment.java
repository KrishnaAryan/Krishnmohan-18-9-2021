import java.util.*;
 ...
 class HrTeam {
        ...
        int result=elements1(array, N, M);

        System.out.println("Number of the employees:"+M);

        int startindex=elements2(result,array,N,M);
    ...
    static int elements1(int array[], int N, int M)
    {
    ...
    static int elements2(int res,int array[], int N, int M)
    {
        int result = Integer.MAX_VALUE;
        for(int i=0; i<=N; i++){
            result = Math.min(result, array[i + M - 1] - array[i]);
